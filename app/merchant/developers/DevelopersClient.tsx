...
export type DeveloperSnippets = {
  createCheckout: ReactNode
  getCheckout: ReactNode
  verify: { node: ReactNode; php: ReactNode; python: ReactNode }
}
...
export default function DevelopersClient({ snippets }: { snippets: DeveloperSnippets }) {
  const [verifyLang, setVerifyLang] = useState<(typeof VERIFY_TABS)[number]["key"]>("node")
      ...
      <div className="max-w-4xl mx-auto px-4 space-y-8">
        ...
        <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
          ...
          <div>
            ...
            {snippets.createCheckout}
          </div>
          <div>
            <p className="font-medium mb-2">2. Status prüfen</p>
            {snippets.getCheckout}
          </div>
          <div>
            <p className="font-medium mb-2">3. Webhook-Signatur prüfen</p>
            <div className="flex gap-1 mb-2" role="tablist">
              {VERIFY_TABS.map((tab) => (
                <button
                  key={tab.key}
                  role="tab"
                  aria-selected={verifyLang === tab.key}
                  onClick={() => setVerifyLang(tab.key)}
                  className={`px-3 py-1.5 rounded-md text-sm font-medium ${
                    verifyLang === tab.key ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                  }`}
                >
                  {tab.label}
                </button>
